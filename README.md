...
An MCP access denial pauses discovery until a successful `/login litellm` (SSO or API key). The pause survives restarts via an empty `~/.pi/agent/litellm-mcp-paused` marker; automatic token refresh does not clear it. Model discovery and chat continue normally. Grant MCP access on the proxy before signing in again. An explicit `litellm.mcp.enabled: false` setting remains disabled after login.
